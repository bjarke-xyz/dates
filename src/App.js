...
function App() {
  ...
  const useLocalTimezoneChange = (e) => {
...
function OaDates({ useLocalTimezone }) {
  ...
  const fromOaDateValueChange = (e) => {
    ...
    if (e.target.value) {
      ...
      const output = OADateToDate(e.target.value, offset).toISOString();
...
function NaturalDates({ useLocalTimezone }) {
  const parse = async (input) => {
    const apiUrl = process.env.REACT_APP_DATES_API_URL;
    try {
